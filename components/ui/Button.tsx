...
const VARIANTS: Record<Variant, string> = {
  ...
  secondary: "border border-primary-900/20 bg-white text-primary-900 hover:border-primary-900/40",
...
function WalletIcon() {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className="h-4 w-4">
      <path strokeLinecap="round" strokeLinejoin="round" d="M3 7a2 2 0 0 1 2-2h13a1 1 0 0 1 1 1v2M3 7v10a2 2 0 0 0 2 2h14a1 1 0 0 0 1-1v-4M3 7l3-3h9M15 13h3a1 1 0 0 0 1-1v-1a1 1 0 0 0-1-1h-3a1 1 0 0 0 0 4z" />
    </svg>
  );
}
...
export function Button({
  ...
  if (disabled) {
    return (
      <span
        aria-disabled
        className={`${base} ${widthClass} cursor-not-allowed bg-primary-900/10 text-primary-900/40 ${className}`}
      >
        {children}
      </span>
    );
  ...
  return (
    <Link href={href} className={`${base} ${VARIANTS[variant]} ${widthClass} ${className}`} {...rest}>
      {children}
