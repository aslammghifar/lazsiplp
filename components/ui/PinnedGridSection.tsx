...
function PinBadge() {
  return (
    <span
      ...
      className="absolute -left-2 -top-2 z-10 flex h-7 w-7 rotate-[-20deg] items-center justify-center rounded-full bg-secondary-500 text-white shadow-md shadow-secondary-900/30"
    >
...
export function PinnedGridSection<T>({
  ...
  if (pinnedItems.length === 0 && gridItems.length === 0) {
    return <p className="text-sm text-primary-800/60">{emptyLabel}</p>;
  }

  return (
    <div className="flex flex-col gap-8">
      {visiblePinnedItems.length > 0 && (
        <div className="scrollbar-hide -mx-4 flex snap-x snap-mandatory gap-6 overflow-x-auto px-4 pt-2">
