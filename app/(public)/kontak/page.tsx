...
export const metadata: Metadata = {
  title: "Kontak — LAZSIP",
  description: "Alamat kantor, jam operasional, dan kontak langsung LAZSIP.",
};

export default function KontakPage() {
  return (
    <div className="mx-auto max-w-2xl px-4 pb-20 pt-28 sm:px-6 sm:pt-32">
      <BackLink href="/">Kembali ke Beranda</BackLink>
      ...
      <p className="mt-3 max-w-xl text-base leading-relaxed text-secondary-700 sm:text-lg">
        Ada pertanyaan seputar zakat, donasi, atau program kami? Chat langsung lewat WhatsApp, atau
        kunjungi kantor kami di jam operasional berikut.
      </p>
      ...
      <div className="mt-10 flex flex-col gap-5">
        <div className="rounded-2xl border border-primary-100 bg-white p-6">
          ...
          <p className="mt-3 text-sm leading-relaxed text-primary-800/70">{siteConfig.address}</p>
        </div>
        ...
        <div className="rounded-2xl border border-primary-100 bg-white p-6">
          ...
          <ul className="mt-3 flex flex-col gap-2 text-sm">
            {siteConfig.jamOperasional.map((j) => (
              <li key={j.hari} className="flex items-center justify-between gap-3 text-primary-800/70">
                <span>{j.hari}</span>
                <span className="font-semibold text-primary-900">{j.jam}</span>
              </li>
            ))}
          </ul>
        </div>
        ...
        <div className="rounded-2xl border border-primary-100 bg-primary-50/60 p-6">
          <h2 className="text-base font-bold text-primary-900">Kontak Langsung</h2>
          <ul className="mt-3 flex flex-col gap-2 text-sm text-primary-800/75">
